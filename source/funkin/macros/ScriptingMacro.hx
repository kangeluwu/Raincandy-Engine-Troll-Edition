	...
	macro public static function addScriptingCallbacks(toInject:Array<String>, ?folder:String = 'states'):Array<Field>
        {
            ...
            */
            ...
            var classSpilt:Array<String> = localClass.toString().split(".");
            var folderName:String = classSpilt.join("/");
            var classSpiltAlt:Array<String> = classSpilt.copy();
            classSpiltAlt.remove(classSpilt[classSpiltAlt.length - 1]);
            var scriptFolder:String = classSpiltAlt.join("/");
            ...
            */
            ...
            fields.findByName("_startExtensionScript") != null ? {/*trace('$fullName yeehaw');*/} : fields.push({
                ...
                kind: FFun({
                    args: [{name: "folder"}, {name: "scriptName"}, {name: "scriptPath"}],
                    expr: macro {
                        for(fileExt in Paths.HSCRIPT_EXTENSIONS){
                            if (_extensionScript != null)break;
                            var fileName = '$scriptName.$fileExt';
                            var folderShits = scriptPath.toString().split(".").join("/");
                            for (fol in [folder,folder+'/'+folderShits]){
                            for (filePath in Paths.getFolders(fol))
                            {
                                ...
                                if (Paths.exists(filePath + fileName))
                                {
                                    ...
                                    break;
                                }else if (Paths.exists(filePath + scriptPath+'.'+fileName)){
                                    var path = filePath + scriptPath+'.'+fileName;
                                    _extensionScript = funkin.scripts.FunkinHScript.fromFile(path, path, _getScriptDefaultVars());
                                    _extensionScript.call("new", []);
                                    break;
                                }
                ...
                switch (constructor.kind)
                {
                    case FieldType.FFun(func):
                        ...
                        body.push(macro {
                            ...
                            _startExtensionScript($v{folder+"/extension"}, $v{className}, $v{scriptFolder});
                        });
