...
**/
class VanillaHUD extends BaseHUD
{
	...
	var engineStringLong = 'Raincandy Engine ${Main.Version.displayedVersion}';
	var engineStringShort = 'RCE ${Main.Version.displayedVersion}';
   ...
   /* override function changedCharacter(id:Int, char:Character){
	...
	override function update(elapsed:Float)
	{
		...
		var shownScore:Float;
		var isHighscore:Bool;
		if (ClientPrefs.showWifeScore){
			shownScore = Math.floor(totalNotesHit * 100);
		}else{
			shownScore = score;
		}
		var commaSeparated = true;
		scoreTxt.text = '$scoreString: ' + FlxStringUtil.formatMoney(shownScore, false, commaSeparated);
