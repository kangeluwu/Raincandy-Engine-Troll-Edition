...
import funkin.objects.NoteSplash.NoteSplashConfig;
...
class NoteSplashDebugSubState extends MusicBeatSubstate
{
	...
	public function new() {
		...
		for (i in 0...maxNotes)
		{
			var x = i * 220 + 240;
			var y = 290;
			var note:StrumNote = new StrumNote(x, y, i%4);
			note.alpha = 0.75;
		...
		add(savedText);
		loadFrames(PlayState.splashSkin);
		changeSelection();
		cameras = [daCam];
	}
	...
	function loadFrames(?tex:String = '')
	{
		if (tex == '' || tex == null || (!Paths.exists('images/'+tex+'.png') && !Paths.exists('images/'+tex+'.xml')))
			tex = NoteSplash.defaultNoteSplash;
		texturePath = tex;
		splashes.forEachAlive(function(spr:FlxSprite) {
	...
	function saveFile()
	{
		...
		var pathSplit:Array<String> = (Paths.getPath('images/$texturePath.png').split('.png')[0] + '.txt').split(':');
