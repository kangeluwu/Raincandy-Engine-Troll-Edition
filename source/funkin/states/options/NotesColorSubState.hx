...
class NotesColorSubState extends MusicBeatSubstate
{
	...
	public function new() {
        ...
        var blackBG = new FlxSprite(230 - 25).makeGraphic(870, 200, FlxColor.BLACK);
		blackBG.alpha = 0.4;
		add(blackBG);
		...
		var text:Alphabet = new Alphabet(150, 86, 'CTRL', true);
		text.setScale(0.4);
		add(text);
		...
		FlxG.mouse.visible = !ClientPrefs.controllerMode;
		controllerPointer.visible = ClientPrefs.controllerMode;
	...
	function updateTip()
	{
		var key:String = !ClientPrefs.controllerMode ? 'Shift' : 'Left Shoulder Button';
	...
	override function update(elapsed:Float) {
		...
		if(FlxG.gamepads.anyJustPressed(ANY)) ClientPrefs.controllerMode = true;
		else if(FlxG.mouse.justPressed || FlxG.mouse.deltaScreenX != 0 || FlxG.mouse.deltaScreenY != 0) ClientPrefs.controllerMode = false;
		//
		...
		if(ClientPrefs.controllerMode && (changedToController || FlxG.gamepads.anyInput()))
		{
		...
		var controllerPressed:Bool = (ClientPrefs.controllerMode && controls.ACCEPT);
		//
		...
		if(generalPressed)
		{
			...
			{
				hexTypeNum = 0;
				for (letter in alphabetHex.lettersArray)
				{
		...
		else if(controls.RESET && hexTypeNum < 0)
		{
			if(FlxG.keys.pressed.SHIFT || FlxG.gamepads.anyPressed(LEFT_SHOULDER))
			{
				for (i in 0...3)
				{
					var strumRGB:ColorSwap = myNotes.members[curSelectedNote].colorSwap;
                   
					var color:FlxColor = ClientPrefs.blankRGB()[curSelectedNote][i];
					switch(i)
					{
						case 0:
			...
			setShaderColor(ClientPrefs.blankRGB()[curSelectedNote][curSelectedMode]);
			FlxG.sound.play(Paths.sound('cancelMenu'), 0.6);
			updateColors();
		}
	}
	...
	{
		if (!ClientPrefs.controllerMode) return FlxG.mouse.overlaps(obj,daCam);
		return FlxG.overlap(controllerPointer, obj);
	}

	function pointerX():Float
	{
		if (!ClientPrefs.controllerMode) return FlxG.mouse.x;
		return controllerPointer.x;
	}
	function pointerY():Float
	{
		if (!ClientPrefs.controllerMode) return FlxG.mouse.y;
		return controllerPointer.y;
	}
	function pointerFlxPoint():FlxPoint
	{
		if (!ClientPrefs.controllerMode) return FlxG.mouse.getScreenPosition(daCam);
	...
	function centerHexTypeLine()
	{
		...
		if(hexTypeNum > 0)
		{
			var letter = alphabetHex.lettersArray[hexTypeNum-1];
			hexTypeLine.x = letter.x - letter.offset.x + letter.width;
		}
		else
		{
			var letter = alphabetHex.lettersArray[0];
	...
	public function spawnNotes()
	{
		...
		{
			Note.initializeGlobalRGBShader(i,onPixel);
        ...
        bigNote.loadGraphic(graphic, true, Math.floor(graphic.width / 4), Math.floor(graphic.height / 5));
	...
	function updateColors(specific:Null<FlxColor> = null)
	{
		...
		for (letter in alphabetHex.lettersArray) letter.color = color;
