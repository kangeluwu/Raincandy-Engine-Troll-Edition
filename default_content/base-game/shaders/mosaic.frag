...
void main()
{
    ...
    if (size > 0) { // bigger than 640x360
        ...
        if (size > 1) {// checking for dumbas, must be > 1280x720 
            ...
            if (gl_FragColor == flixel_texture2D(bitmap, (floor(openfl_TextureCoordv * blocks) + offset - vec2(0.25, 0)) / blocks)) {
                offset += vec2(0.25, 0);
            }
            // if (gl_FragColor == texture2D(bitmap, (floor(openfl_TextureCoordv * blocks) + offset - vec2(0, 0.25)) / blocks)) {
            //     offset += vec2(0, 0.25);
            // }
            gl_FragColor = flixel_texture2D(bitmap, (floor(openfl_TextureCoordv * blocks) + offset) / blocks);
        }
    } else
